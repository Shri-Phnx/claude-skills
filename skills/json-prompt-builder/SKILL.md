...
If unclear, ask: *"Is this for extraction, scoring, classification, content generation, or an agent pipeline node?"*
...
When building a JSON prompt using this skill, always deliver:

1. **System Prompt block** — complete, ready to paste
2. **User Prompt block** — with placeholders clearly marked
3. **Example call** — one filled input/output pair
4. **API settings note** — temperature, max_tokens, seeding tip
5. **Validation loop note** — error handling pattern for the workflow

Offer to generate the prompt as a downloadable `.md` file for workflow documentation.
